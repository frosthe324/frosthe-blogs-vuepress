...
对于 Agentic Coding 而言，模型能力决定上限，而上下文工程决定下限。真正影响日常体验的，往往不是模型能力在 Benchmark 上多出的几分，而是 Agent 在当前任务里是否拿到了**正确、干净的上下文**。

从实践角度看，可以把四类上下文理解为一套协作分工:

- 提示词负责描述当前任务。
- 会话历史负责维持短期连续性。
- 指令负责沉淀稳定规则与工作范式。
- 记忆负责保存项目知识、架构决策与经验案例。

因此，构建上下文的关键并不是一次性把所有内容塞进窗口，而是让信息以合适的层级被组织起来，并在需要时被准确加载。临时问题留在 Prompt 中，稳定约束上升为 Instructions，可复用经验沉淀为 Memory，再配合渐进揭露 (Progressive Disclosure) 控制上下文密度，才能在成本、效果与可维护性之间取得更好的平衡。

当我们开始把 Agent 当作真正的协作对象，而不是单纯的聊天模型时，"如何管理上下文" 就会成为和 "如何选择模型" 同等重要，甚至更重要的工程问题。